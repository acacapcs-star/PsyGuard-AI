...
12 階刻度,各有顏色與標籤(狀態良好 → 需立即協助)。升降不對稱:
...
| Check-in | 情緒穩定度、情緒輕鬆度、情緒韌性值三支滑桿 + 備註;儲存後 ERS 卡自動展開。另有歷史頁 |
...
| Toolbox | 四個工具:自我對話、4-7-8 呼吸、5-4-3-2-1 grounding、情緒字典;含使用紀錄 |
...
| Export Report | 摘要匯出:JSON 或 PNG,可選天數區間 |
| Settings | 見下節 |
...
苗豐強科技創新獎不分組別、就全體參賽作品評選,714 取 3。這是 lii 目前最強的第三方外部驗證,也是 v1 階段的成果——**v2 lii 由作者獨立重建,尚未有對應的獎項結果**。
...
> 需要說明的是:這份公文取得於 v1 PsyGuard 團隊階段,由團隊具名請求。v2 lii 為作者後續獨立重建,設計上沿用了同一份法規對照,並在此基礎上加深(例如把「分層授權」實作成三張結構分離的資料表、把「避免標籤化」實作成紅燈時撤下排名)。
